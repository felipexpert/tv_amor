...
def cortar_silencio_final(audio: AudioSegment, margem_ms: int = 35) -> AudioSegment:
  return audio

def cortar_silencio_final_bkp(audio: AudioSegment, margem_ms: int = 35) -> AudioSegment:
    """
...
async def generate_audio(audio_request: AudioRequest, file_path: Path) -> AudioInfo:
    ...
    audio = cortar_silencio_final(audio, margem_ms=35)
