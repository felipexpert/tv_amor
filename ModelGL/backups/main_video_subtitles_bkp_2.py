...
BURN_IN = True         # True = "queima" a legenda no vídeo (-vf subtitles)
# ====================
...
def legendar_video(video_path: str, model_name: str = "small"):
    ...
    # -------------- embutir legendas (QUEIMAR) ---------------
    print("[3/3] Criando vídeo com legenda 'queimada'...")
    try:
        if BURN_IN:
            # Tenta usar filtro subtitles (queima a legenda). Convertendo caminho para posix evita problemas com backslash
            # Aqui está o ajuste principal: o comando FFmpeg agora tem um arquivo de saída diferente do de entrada.
            ...
            subprocess.run([
                ...
                str(output_path) # O arquivo de saída AGORA é diferente do arquivo de entrada
            ], check=True)
        else:
            # Caso a intenção fosse 'softsub', que não é o seu caso, mas o código original tinha
            # A lógica é similar, garante que o output não seja o mesmo que o input
            subprocess.run([
                "ffmpeg",
                "-i", str(video),
                "-i", str(srt_path),
...
def copy_video(video_path: str) -> str | None:
    ...
    if not video_file.is_file():
        ...
        # Cria o nome do novo arquivo (com '_legenda' para indicar que é um arquivo temporário)
...
if __name__ == "__main__":
    ...
    if new_video_path:
        ...
        # Opcional: remover o arquivo intermediário após o processo, se desejado.
        # os.remove(new_video_path)
        # print(f"Arquivo de legenda {new_video_path} removido.")