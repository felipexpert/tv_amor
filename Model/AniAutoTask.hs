...
import Model.EpisodePersona (EPeNumber(..), EPeLabel(..))
...
episodeToTaskDialoguesIO episodeComplete = do
  -- edp = fmap dialoguePeToTaskDialogue (E.eDialoguePeList ep)
  ...
  return res
  where
    richTextWithPeTuple :: E.EDialoguePe -> [(E.DRichText, EPeLabel)]
